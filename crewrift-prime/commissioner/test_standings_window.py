...
The Competition "Standings" board grades players on RECENT merit — only rounds
whose gameplay completed within the last ``STANDINGS_WINDOW_HOURS`` hours (default
6) count toward the win rate and cumulative score. These tests assert, for BOTH
publishing paths that share ``_win_total_board``:
...
class RankDivisionWindowTest(unittest.TestCase):
    def test_default_window_is_six_hours(self) -> None:
        self.assertEqual(comm.STANDINGS_WINDOW_HOURS, 6.0)

    def test_stale_rounds_are_dropped_from_standings(self) -> None:
        """A player whose only wins are older than 6h drops to a 0 win rate."""
        commissioner = _commissioner()
        now = datetime.now(UTC)
        recent_round, stale_round = uuid4(), uuid4()
        # ply_recent won 1/1 in a round 1h ago (in-window).
        # ply_stale won 1/1 in a round 10h ago (outside the 6h window).
        round_results = [
    ...
    def test_window_disabled_keeps_all_rounds(self) -> None:
        """CREWRIFT_PRIME_STANDINGS_WINDOW_HOURS=0 reverts to the all-time board."""
        commissioner = _commissioner()
        now = datetime.now(UTC)
        stale, recent = uuid4(), uuid4()
        round_results = [
        ...
        try:
            comm.STANDINGS_WINDOW_HOURS = 0.0
            snapshots = commissioner.rank_division(_ctx(completed, round_results))
        finally:
...
class RoundCompleteWindowTest(unittest.TestCase):
    ...
    def test_stale_history_rows_excluded_from_published_board(self) -> None:
        commissioner = _commissioner()
        now = datetime.now(UTC)
        # Seed persisted history: a stale row (10h ago) and a recent row (1h ago)
